...
const sqlGetPhotosCount = async () =>
  sql`
  SELECT COUNT(*) FROM photos
  WHERE hidden IS NOT TRUE
`.then(({ rows }) => parseInt(rows[0].count, 10))

const sqlGetPhotosCountIncludingHidden = async () =>
  sql`
  SELECT COUNT(*) FROM photos
`.then(({ rows }) => parseInt(rows[0].count, 10))

const sqlGetPhotosTagCount = async (tag: string) =>
  sql`
  SELECT COUNT(*) FROM photos
...
const sqlGetPhotosCameraCount = async (camera: Camera) =>
  sql`
  SELECT COUNT(*) FROM photos
  WHERE
...
const sqlGetPhotosFilmSimulationCount = async (simulation: FilmSimulation) =>
  sql`
  SELECT COUNT(*) FROM photos
...
const sqlGetPhotosDateRange = async () =>
  sql`
...
const sqlGetPhotosTagDateRange = async (tag: string) =>
  sql`
  SELECT MIN(taken_at_naive) as start, MAX(taken_at_naive) as end
  FROM photos
  WHERE ${tag}=ANY(tags) AND
...
const sqlGetPhotosCameraDateRange = async (camera: Camera) =>
  sql`
  SELECT MIN(taken_at_naive) as start, MAX(taken_at_naive) as end
  FROM photos
  WHERE
...
export const getCoverPhotos = async () => {
  return safelyQueryPhotos(async () => {
    const client = await db.connect()
    const ids = heroSectionImgs.map((id) => `'${id}'`).join(", ")
    return client
...
export const getPhotosNearId = async (id: string, limit: number) => {
...
export const getPhotosDateRange = () => safelyQueryPhotos(sqlGetPhotosDateRange)
...
export const getPhotosFilmSimulationDateRange = (simulation: FilmSimulation) =>
  safelyQueryPhotos(() => sqlGetPhotosFilmSimulationDateRange(simulation))
